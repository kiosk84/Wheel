      ...
      {showSuccessModal && (
        <div className="fixed inset-0 flex items-center justify-center bg-black bg-opacity-60 z-50">
          <div className="bg-gray-900 p-6 rounded-lg shadow-lg max-w-xs w-full text-center">
            <h4 className="text-lg font-bold text-green-400 mb-3">Заявка отправлена!</h4>
            <p className="text-white mb-4">Вы добавлены в список ожидающих подтверждения.<br/>Ожидайте одобрения от администратора.</p>
            <button
              className="bg-green-600 hover:bg-green-500 text-white font-semibold rounded px-6 py-2 mt-2"
              onClick={() => { setShowSuccessModal(false); onCloseAction(); }}
            >
              Ок
            </button>
          </div>
